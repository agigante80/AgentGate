...
4. ~~**`docker-compose.yml.example` update**~~ — ✅ Resolved: the example now shows commented `image:` alternatives for registry pulls (`:latest`, pinned stable, `:develop`, pinned dev) alongside the default `build: .` option.
